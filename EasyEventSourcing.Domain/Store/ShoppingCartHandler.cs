...
namespace EasyEventSourcing.Domain.Store
{
        ...
        , ICommandHandler<Checkout>
    {
        ...
        {
            this.repo.Save(ShoppingCart.Create(Guid.NewGuid(), cmd.ClientId));
        }

        public void Handle(AddProductToCart cmd)
        {
            Execute(cmd.CartId, (cart) => cart.AddProduct(cmd.ProductId, cmd.Quantity));
        }

        public void Handle(RemoveProductFromCart cmd)
        {
            Execute(cmd.CartId, (cart) => cart.RemoveProduct(cmd.ProductId, cmd.Quantity));
        }
